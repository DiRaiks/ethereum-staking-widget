...
import {
  ...
  type EarnStateValue,
} from './earn-state-utils';
export { EARN_STATES, computeEarnRuntimeState } from './earn-state-utils';
export type { EarnStateValue } from './earn-state-utils';
 ...
 * - **partial**: Only specific vaults are shown via URL whitelist (e.g., `?earn=vault1,vault2`)
...
export const useEarnState = () => {
  ...
  const isVaultEnabled = (vaultName: string) => {
    ...
    // In PARTIAL state: vault is enabled only if it IS in the URL whitelist
  ...
  return {
    ...
    isEarnDisabled,
    isEarnPartial,
    isEarnVaultsWhitelistedByURL: someVaultsEnabledByURL,
    isVaultEnabled,
    isVaultDisabled,
  };
};
